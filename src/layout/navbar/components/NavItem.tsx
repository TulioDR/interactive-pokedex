type Props = {
   icon: string;
   text: string;
   isSelected?: boolean;
   favorite?: true;
};

export default function NavItem({ icon, text, isSelected, favorite }: Props) {
   return (
      <button
         className={`h-full rounded-lg px-5 flex justify-center items-center gap-1 relative cursor-pointer bg-white shadow-md
            ${
               favorite
                  ? "text-[#E60012] hover:bg-[#E60012] hover:text-white active:bg-[#E60012] active:text-white"
                  : "text-black/60 hover:bg-slate-800 hover:text-white active:bg-slate-800 active:text-white"
            }   
         `}
      >
         <span
            style={{ fontVariationSettings: `'FILL' ${isSelected ? 1 : 0}` }}
            className="material-symbols-rounded text-3xl!"
         >
            {icon}
         </span>
         <span className="font-bold text-sm">{text}</span>
      </button>
   );
}
